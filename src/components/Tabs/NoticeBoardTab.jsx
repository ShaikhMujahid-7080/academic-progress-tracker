...
export function NoticeBoardTab({ selectedStudent, semester }) {
  ...
  const handleScrollToNotice = (noticeId) => {
    // Automatically switch to Pinned category when clicking from the pinned sidebar
    setActiveCategory('pinned');

    // Wait for the tab to switch and notice to render
    setTimeout(() => {
      const element = document.getElementById(`notice-${noticeId}`);
      if (element) {
        element.scrollIntoView({ behavior: 'smooth', block: 'center' });
        // Add highlight effect
        element.classList.add('ring-2', 'ring-orange-400', 'ring-offset-2', 'rounded-2xl');
        setTimeout(() => {
          element.classList.remove('ring-2', 'ring-orange-400', 'ring-offset-2', 'rounded-2xl');
        }, 2000);
      }
    }, 100);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Header */}
      <div className="flex flex-col gap-4 sticky top-[136px] sm:top-[128px] z-20 bg-gray-50/95 backdrop-blur supports-[backdrop-filter]:bg-gray-50/60 py-4 transition-all -mx-4 px-4 rounded-b-2xl mb-4 border-b border-gray-100 shadow-sm">
        <div className="flex items-center justify-between w-full">
          <div className="min-w-0">
            <h2 className="text-xl sm:text-3xl font-bold text-gray-900 mb-0.5 sm:mb-1 flex items-center gap-2 sm:gap-3">
            ...
            <p className="text-[10px] sm:text-sm text-gray-500 font-medium truncate">
              {canManageNotices
                ? `${isAdmin ? 'Admin' : 'Co-Leader'}: Management Access`
        ...
        <div className="flex p-1 bg-gray-200/50 rounded-xl w-fit overflow-x-auto max-w-full scrollbar-hide border border-gray-200/30">
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => setActiveCategory(cat.id)}
              className={`
                flex items-center gap-2 px-3 py-1.5 sm:px-6 sm:py-2.5 rounded-lg transition-all text-[10px] sm:text-sm font-bold whitespace-nowrap
              ...
              `}
            >
              <cat.icon className={`w-3 h-3 sm:w-4 sm:h-4 ${activeCategory === cat.id ? cat.activeClass : ''}`} />
              <span>{cat.label}</span>
              {cat.count > 0 && (
                <span className={`
                  px-1.5 py-0.5 rounded-full text-[8px] sm:text-xs min-w-[18px] text-center
                  ${activeCategory === cat.id ? cat.badgeClass : 'bg-gray-300/50 text-gray-600'}
                `}>
                  {cat.count}
                </span>
              )}
